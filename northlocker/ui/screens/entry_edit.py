...
def build_entry_edit(entry):
    state = {'show_pass': False}
    
    title_field = TextArea(text=entry.title, multiline=False)
    user_field = TextArea(text=entry.username, multiline=False)
    pass_field = TextArea(text=entry.password, multiline=False, password=Condition(lambda: not state['show_pass']))
    url_field = TextArea(text=entry.url, multiline=False)
    totp_field = TextArea(text=entry.totp_secret or "", multiline=False)
    notes_field = TextArea(text=entry.notes)
    ...
    def _(event):
        if not title_field.text.strip():
            return
        entry.title = title_field.text.strip()
        entry.username = user_field.text.strip()
        entry.password = pass_field.text
        entry.url = url_field.text.strip()
        entry.totp_secret = totp_field.text.strip() or None
        entry.notes = notes_field.text.strip()
